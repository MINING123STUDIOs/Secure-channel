...
`script.js` (all logic — crypto, UI wiring, import/export).
...
2. XOR every byte against a fixed 12-byte constant (`ENVELOPE_MASK`) —
   this isn't a secret and isn't meant to be one; it exists only to break
   the recognizable `{"` / `eyJ` byte pattern,
...
explained in the comment block at the top of `script.js`:
...
wired via `onclick` handlers calling into `script.js`.
...
### `script.js`
Roughly four regions, top to bottom:
1. **Constants & helpers** — thresholds (§4 table), `yieldToUI`,
   `maskBytes`/`encodeOpaquePacket`/`decodeOpaquePacket` (§3.1), base64/byte
   utilities.
2. **Crypto core** — `generateIdentityKeyPair`, `generateDHKeyPair`, `dh`,
   `kdfRK`, `kdfCK`, `deriveMessageAesKey`, `fingerprintOf`,
   `computeInitialSharedSecret`, and the `RatchetSession` class
   (`encrypt`/`decrypt`) described in §2.
3. **Large-payload helpers** — `bytesToBase64Chunks`/`base64ChunksToBytes`,
   `buildLargeExportParts`/`streamingParseLargeFile` (§3.2, §4).
4. **UI layer** — theme toggle, the `Progress` indicator module, status/
   copy/export/import helpers, key generation & identity import/export,
   the show/hide toggle, and the top-level `doEncrypt`/`doDecrypt` handlers
...
who has this source code and therefore knows `ENVELOPE_MASK` (§3.1 is not a
...
GPL-3.0, per the header in `index.html`/`style.css`/`script.js`. See
